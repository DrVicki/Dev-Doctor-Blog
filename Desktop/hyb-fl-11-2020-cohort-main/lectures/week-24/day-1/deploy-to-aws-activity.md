...
- April 2021: Lachlan snaked the whole document and made it available here
...
Select the "Free tier eligible" hardware choice or _instance type_ (this will specify the size of the memory, CPU, network performance, storage, etc.):
...
Note from the above that you need to provide the relative path to your `.pem` file. The new line in your `.bash_profile` should look as follows (except with _your_ `.pem` file name and `ubuntu@` address):
...
Copy _all_ of the output text, go back to the GitHub page you left open in your browser (for adding an SSH key), enter a reasonable title for your key (e.g., `ec2-aws-instance-2019-11`), and then paste your copied text into the key field:
...
**Note:** You must have a purchased domain name to do this step. You can buy one from [Namecheap](https://www.namecheap.com/), [GoDaddy](https://www.godaddy.com/), or a host (ha!) of other domain providers. If you do not have a domain name, then you can still view a _single_ site on your EC2 instance by using the EC2-provided IP address. If, however, you want to set up subdomains, then a domain name is required. If you want to use HTTPS, which is strongly recommended and increasingly necessary, then you will need a certificate, and a domain name is required to use a certificate.
...
>
> A blank record (sometimes seen as the ‘@’ record) points your main domain to a server. You can also set subdomains to point to other IP addresses as well, if you run multiple webservers. Finally, a wildcard record, shown usually as ‘_’ or ‘_.yourdomain.com,’ acts as a catch-all record, redirecting every subdomain you haven’t defined elsewhere to an IP address.
>
...
You should _hopefully_ see something like the following:
...
We will now follow the steps provided by Certbot to obtain our certificate (execute each command _one by one_; that is, do not copy and paste all of the commands and try to execute them all at once).
...
If you have already done this, then you can simply _update_ your certificate. Updates need to happen anytime you add a domain or subdomain to the `sites-available/default` file. Simply SSH into your EC2 server as above and run the following commands:
